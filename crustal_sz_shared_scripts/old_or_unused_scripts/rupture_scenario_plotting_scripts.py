...
from helper_scripts import read_rupture_csv, make_total_slip_dictionary, get_figure_bounds
...
def vertical_disp_figure(all_ruptures_disp_dict, NSHM_directory, target_rupture_ids, extension1, slip_taper,
    ...
    for rupture_id in target_rupture_ids:
        ...
        fig.suptitle(f"Rupture {rupture_id}{extension3}\n{model_version}")
        fig.tight_layout()

        # make folder for outputs
        if not os.path.exists(f"../{results_version_directory}/{extension1}"):
            os.mkdir(f"../{results_version_directory}/{extension1}")
        if not os.path.exists(f"../{results_version_directory}/{extension1}/scenario_displacements"):
            os.mkdir(f"../{results_version_directory}/{extension1}/scenario_displacements")

        if not os.path.exists(f"../{results_version_directory}/{extension1}/rupture_geojson"):
            os.mkdir(f"../{results_version_directory}/{extension1}/rupture_geojson")

        # save figure and geojson with slip data
        for file_type in file_type_list:
            fig.savefig(f"../{results_version_directory}/{extension1}/scenario_displacements/"
                        f"rupture_{rupture_id}{extension3}.{file_type}", dpi=300)
        ruptured_discretized_polygons_gdf.to_file(
            f"../{results_version_directory}/{extension1}/rupture_geojson/scenario_slip_{rupture_id}{extension3}.geojson",
            driver="GeoJSON")
