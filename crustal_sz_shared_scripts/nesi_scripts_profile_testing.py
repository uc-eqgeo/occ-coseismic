...
def prep_cumu_PPE_NESI_outdated(model_version_results_directory, branch_site_disp_dict, extension1, 
                       hours : int = 0, mins: int= 3, mem: int= 45, cpus: int= 1, account: str= 'uc03610',
                       time_interval: int = 100, n_samples: int = 1000000, sd: float = 0.4, S="_"):
    """
    Must first run get_site_disp_dict to get the dictionary of displacements and rates
    ...
    with open(f"../{model_version_results_directory}/cumu_PPE_slurm_task_array.sl", "wb") as f:
        ...
        if S:
            f.write(f"python nesi_scripts.py --site `awk 'FNR == ENVIRON[\"SLURM_ARRAY_TASK_ID\"] {{print $1}}' {site_file}` --branchdir `awk 'FNR == ENVIRON[\"SLURM_ARRAY_TASK_ID\"] {{print $2}}' {site_file}` --time_interval {int(time_interval)} --n_samples {int(n_samples)} --sd {sd} --scaling {S}\n\n".encode())
        else:
            f.write(f"python nesi_scripts.py --site `awk 'FNR == ENVIRON[\"SLURM_ARRAY_TASK_ID\"] {{print $1}}' {site_file}` --branchdir `awk 'FNR == ENVIRON[\"SLURM_ARRAY_TASK_ID\"] {{print $1}}' {site_file}` --time_interval {int(time_interval)} --n_samples {int(n_samples)} --sd {sd}\n\n".encode())
...
site_of_interest='1190_4773'
branch_results_directory='../results/crustal_Model_CFM_50km_nesi/sites_c_MDA2'
...
print(f"Running site {site_of_interest} with {n_samples} samples for {investigation_time} years....")
...
lap = time()
print(f'Laptime 8: {lap-begin:.2f} seconds')
begin=time()
...
lap = time()
print(f'Laptime 9: {lap-begin:.2f} seconds')
begin=time()
...
for threshold in thresholds:
    ...
    n_exceedances_total_abs[thresholds == threshold] = np.sum(np.abs(cumulative_disp_scenarios) > threshold)
    n_exceedances_up[thresholds == threshold] = np.sum(cumulative_disp_scenarios > threshold)
for threshold in thresholds_neg:
    n_exceedances_down[thresholds_neg == threshold] = np.sum(cumulative_disp_scenarios < threshold)
