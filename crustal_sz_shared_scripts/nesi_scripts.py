...
def prep_SLURM_weighted_sites_submission(out_directory, tasks_per_array, n_tasks, site_file,
    ...
    with open(slurm_file, "wb") as f:
        f.write("#!/bin/bash -e\n".encode())
        f.write(f"#SBATCH --job-name=06_{os.path.basename(out_directory)}\n".encode())
        ...
        f.write(f"#SBATCH -o logs/06_{os.path.basename(out_directory)}_weights_%j_task%a.out\n".encode())
        f.write(f"#SBATCH -e logs/06_{os.path.basename(out_directory)}_weights_%j_task%a.err\n\n".encode())
